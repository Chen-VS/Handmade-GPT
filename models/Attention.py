...
class CausalMultiHeadAttention(nn.Module):
    def __init__(self,d_model : int, num_heads : int,dropout:float=0.3):
        ...
        self.resid_dropour = nn.Dropout(dropout)
    ...
    def forward(self,x:torch.Tensor):
        ...
        casual_mask = torch.triu(torch.ones(T, T, dtype=torch.bool).to(x.device),diagonal = 1)  # [T, T]
        scores = scores.masked_fill(casual_mask.to(scores.device), float('-inf'))
        ...
        output = self.resid_dropour(output)
